...
def link_email(data):
    ...
    dict_list = find_one_module(
        "reference_employee_4", query, reference_remove_dict)
    ver_code = random_with_N_digits(6)
    db.reference_employee_6.update_one({"index": dict_list['index']}, {"$set": {
        "email": data['email'],
...
def register_email(data):
    ...
    if validation1:
        ...
        validation2 = find_one_module(
            "reference_employee_6", query2, reference_remove_dict)
        if validation2:
            content = {
                "receiver": data['email'],
            ...
            sent_verification(content, data)
            db.reference_employee_6.update_one(
                query2, {"$set": {"verified": "Yes"}})
            return("1")
        else:
            return("2")
