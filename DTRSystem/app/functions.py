...
def get_sched_and_area(date_value,index:int):
	...
	latest_dict = get_applicable_record_now('reference_employee_3', index, {'meta':0,'_id':0},date_value)
...
def get_latest_record(date_value,index):
	super_dict = {}
	for x in range(1,5):
		super_dict.update(get_applicable_record_now(f'reference_employee_{x}',index,{"meta":0,"_id":0},date_value))
...
def get_today_sched(reference_code,full_time,branch):
	...
	if not find_one_module('reference_schedule_type', {'code':reference_code,'meta.is_deleted':False}, {'_id':0}):
		reference_code = 'N1'
	sched_cursor = find_module('reference_schedule_type',{'code':reference_code,'meta.is_deleted':False},{'_id':0})
	...
	if not find_rec:
		find_rec = find_one_module('reference_schedule_type', {'code':'N1','meta.is_deleted':False}, {'_id':0})
	
	
	apply_date = dt.strptime(dt.strftime(dt.strptime(find_rec['date'],"%Y-%m-%d"),"%m/%d/%Y"),"%m/%d/%Y")
	...
	allowed_out = dt.strptime(sched_out,"%H:%M") + timedelta(hours=0, minutes=15)
...
def get_date_range(start, date_now, last_day) -> list:
	...
	date_list = [cutoff_start, cutoff_end, prev_start, prev_end]
	return(date_list)
...
def convert_nan_to_date(raw_data):
	if str(raw_data) == "nan":
		...
		data = dt.strftime(date_converter(str(raw_data).replace("'", "")),'%Y-%m-%d')
...
def disapproved_leave(data):
	date_now = dt.now() - timedelta(hours=0, minutes=60)
	date_now = date_now.strftime("%m/%d/%Y")
	db.utilities_leave_application.update_one({"index":int(data['index'])},{"$set":{"basic_info.status":"Disapproved/Cancelled"}})
	cursor = list(db.utilities_leave_detail.find({"ref_index":int(data['index'])}))
	total_hour = 0
	...
	find_rec = get_applicable_record_now('reference_leave_credit', session['index'], {'meta':0,'_id':0},'')
	if find_rec:
...
def list_of_employee(data):
	...
	for x in ['company','department','section']:
		if data[x] == "ALL":
			...
			dict_list[x] = data[x]
	set_data = []
	sort_cursor = db['reference_employee_1'].find({'meta.is_deleted':False},{"meta":0,"_id":0,"applied":0}).sort('last_name')
	for x in sort_cursor:
		dict_list['index'] = x['index']
		
		if db['reference_employee_2'].find_one(dict_list,{"index":1,"_id":0}):
			z = x
			z.update(db['reference_employee_2'].find_one(dict_list,{"index":1,"_id":0}))
			set_data.append(z)
...
def time_conversion(time_value):
	if "+" in time_value:
		converted_time = timedelta(days=1,hours=int(time_value[:2]), minutes=int(time_value.replace('+','')[3:]))
	elif time_value == "":
		converted_time = ""
	else:
		converted_time = timedelta(hours=int(time_value[:2]), minutes=int(time_value.replace('+','')[3:]))
...
def check_difference(dict_1,dict_2,basis,ot_hrs):
	...
	first_in = get_first([dict_1['first_in'],dict_1['last_out'],dict_2['first_in'],dict_2['last_out']])
	last_out = get_last([dict_1['first_in'],dict_1['last_out'],dict_2['first_in'],dict_2['last_out']])
		...
		if dict_1['first_in'] and dict_1['last_out']:
			if dict_1['first_in'] and dict_2['first_in']:
				if time_conversion(dict_1['first_in']) > time_conversion(dict_2['first_in']):
					basic_in += time_diff_undertime(dict_2['first_in'], dict_1['first_in'])
					if time_conversion(basis['sched_in']) < time_conversion(dict_2['first_in']):
						no_of_lates += time_diff_late(time_conversion(dict_2['first_in']), basis['sched_in'])

					else:
						raw_in += time_diff_undertime(dict_2['first_in'],basis['sched_in'])
			...
			if dict_1['last_out'] and dict_2['last_out']:
				if time_conversion(dict_1['last_out']) < time_conversion(dict_2['last_out']):
					basic_out += time_diff_undertime(dict_1['last_out'], dict_2['last_out'])
					if time_conversion(basis['sched_out']) < time_conversion(dict_2['last_out']):
						under_time += time_diff_undertime(time_conversion(dict_2['last_out']), basis['sched_out'])
					else:
						raw_out += time_diff_undertime(basis['sched_out'], dict_2['last_out'])
			...
			if first_in and last_out:
				if  not any(xs in basis['day_remarks'] for xs in remarks):
					...
					if basic >= (basis['working_hour'] * 60):
						...
						basic = ((basis['working_hour'] * 60)-(no_of_lates+under_time))/480 
...
def adjustment_basic(index,date_now,date_end):
	...
	ob_cursor = db['utilities_ob_application'].find({"meta.is_deleted":False,'basic_info.ref_index':index,"basic_info.effective_start":date_now})
	logs_cursor = db['utilities_missing_logs_application'].find({"meta.is_deleted":False,'basic_info.ref_index':index,"basic_info.effective_start":date_now})
	ot_cursor = db['utilities_ot_application'].find({"meta.is_deleted":False,'basic_info.ref_index':index,"basic_info.effective_start":date_now})
	day_remarks = ''
	remarks = ['SH','DO',"RD",'RH']

	collections = {'ob':'ob','missing_logs':'logs','wfh':'wfh','day_offset':'day_offset'}
	for collection in collections:
		...
		for x in cursor:
			if 'offset' in collection:
				basis = get_today_sched(record['schedule_type'], dt.strptime(x['offset_day'],'%Y-%m-%d'), record['area'])
			else:
				basis = get_today_sched(record['schedule_type'], dt.strptime(x['date'],'%Y-%m-%d'), record['area'])
			ot_hrs = 0
			dict_list = {}   
			list_data = []
			time_list = []
			...
			att_dict = db['attendance_record'].find_one({'index':index,'date':x['date']},{'first_in':1,'last_out':1,'_id':0})
			...
			for item in collections:
				col = db[f'utilities_{item}_application'].find_one({'basic_info.ref_index':index,'date':x['date']},{'meta':0,'_id':0})
				if col and dt.strptime(date_now,"%Y-%m-%d") > dt.strptime(col['basic_info']['effective_start'],"%Y-%m-%d") and item !=  'day_offset':  
					pass_cut_off[list(set_data_past).index(x['date'])][f'{collections[item]}_in'] = col[f'{collections[item]}_am']
					pass_cut_off[list(set_data_past).index(x['date'])][f'{collections[item]}_out'] = col[f'{collections[item]}_pm']
				elif col and dt.strptime(date_now,"%Y-%m-%d") == dt.strptime(col['basic_info']['effective_start'],"%Y-%m-%d") and item!= 'day_offset':
					current_cut_off[list(set_data_current).index(x['date'])][f'{collections[item]}_in'] = col[f'{collections[item]}_am']
					current_cut_off[list(set_data_current).index(x['date'])][f'{collections[item]}_out'] = col[f'{collections[item]}_pm']
			...
			if not(dt.strptime(date_now,'%Y-%m-%d') <= dt.strptime(x['date'],'%Y-%m-%d') and dt.strptime(date_end,'%Y-%m-%d') >= dt.strptime(x['date'],'%Y-%m-%d')):			
				basic += check_difference(pass_dict, current_dict,basis,ot_hrs)
		...
		if not any(d['date'] == x['date'] for d in list_data):
			...
			for item in collections:
				...
				if col and 'offset' in item:
					basis = get_today_sched(record['schedule_type'], dt.strptime(col['offset_day'],'%Y-%m-%d'), record['area'])
				else:
					basis = get_today_sched(record['schedule_type'], dt.strptime(x['date'],'%Y-%m-%d'), record['area'])  
