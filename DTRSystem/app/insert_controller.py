...
def insert_reference(data, content):
    ...
    try:
        record_count = count_module(f'reference_{content}', {
                                    "code": data['code'], "meta.is_deleted": False})
    except:
        record_count = count_module(f'reference_{content}', {
                                    "date": data['date'], "meta.is_deleted": False})
...
def update_reference(data, content: str, index: int):
    update_dict = db[f'reference_{content}'].find_one(
        {"index": int(index), "meta.is_deleted": False})
    ...
    if content == "schedule_type":
        if applied_date != data['date']:
            ...
            x = find_one_module(f'reference_{content}', {'index': int(
                index), "meta.is_deleted": False}, {'_id': 0})
            result = db[f'reference_employee_3'].update_many(
                {'schedule_type': x['code']}, {"$set": {'schedule_type': update_dict['code']}})
            result = update_one_module(f'reference_{content}', {"index": int(
                index), "meta.is_deleted": False}, update_dict)
            notification = "Record Successfully Updated"
    else:
        x = find_one_module(f'reference_{content}', {'index': int(
            index), "meta.is_deleted": False}, {'_id': 0})
        if content == 'company':
            result = db[f'reference_employee_2'].update_many(
                {content: x['code']}, {"$set": {content: update_dict['code']}})
        elif content == 'area':
            result = db[f'reference_employee_3'].update_many(
                {content: x['code']}, {"$set": {content: update_dict['code']}})
            result = db[f'reference_holiday'].update_many(
                {'location': x['code']}, {"$set": {'location': update_dict['code']}})
            result = db[f'reference_location'].update_many(
                {'location': x['code']}, {"$set": {'location': update_dict['code']}})
        elif location == 'location':
            result = db[f'reference_employee_3'].update_many(
                {content: x['code']}, {"$set": {content: update_dict['code']}})
        elif content == 'department':
            section_cursor = db[f'reference_section'].find(
                {'department': x['code'], 'meta.is_deleted': False}, {'_id': 0})
            for y in section_cursor:
                result = db[f'reference_employee_2'].update_many({
                    'section': f"{y['department']}-{y['code']}"}, {
                    "$set": {'section': f"{update_dict['code']}-{y['code']}", "department": update_dict['code']}})

            result = db[f'reference_section'].update_many(
                {'department': x['code']}, {"$set": {'department': update_dict['code']}})

        elif content == 'section':
            print(f"{x['department']}-{x['code']}")
            result = db[f'reference_employee_2'].update_many({content: f"{x['department']}-{x['code']}"}, {
                                                             "$set": {content: f"{update_dict['department']}-{update_dict['code']}"}})
...
def insert_reference_employee(data, content):
    ...
    for x in range(1, 6):
        if x == 1:
            insert_dict = dict_getter('default_dict')
            record_count = count_module(f'reference_{content}_1', {
                                        "SN": data['SN_1'], "meta.is_deleted": False})
            if record_count == 0:
...
def update_reference_employee(data, content: str, index: int):
    ...
    for x in range(1, 6):
        ...
        if x == 1:
            update_dict = db[f'reference_{content}_1'].find_one(
                {"index": int(index), "meta.is_deleted": False}, {"_id": 0})
            ...
            update_dict['meta']['reason_edit'] = data['reason_edit']
            update_dict['meta']['edited_by'] = session['user']
            update_dict['meta']['date_edited'] = dt.strftime(
                dt.now(), '%Y-%m-%d %H:%M')
            result = update_one_module(f'reference_{content}_1', {
                                       "index": int(index), "meta.is_deleted": False}, update_dict)
            notification = "Record Successfully Updated"
            ...
            if record_count == 0:
                ...
                insert_dict['meta']['date_edited'] = dt.strftime(
                    dt.now(), '%Y-%m-%d %H:%M')
                insert_dict['applied'] = data['applied'] + " " + date_now
                insert_dict['ref_index'] = int(
                    count_module(f'reference_{content}_{x}', {})) + 1
                insert_dict['index'] = int(index)
                insert_dict['_id'] = ObjectId()
                result = insert_one_module(
                    f'reference_{content}_{x}', insert_dict)
    ...
    for data in reader:
        for x in range(1, 6):
            ...
            if x == 1:
                ...
                if record_count == 0:
                    index = int(count_module(f'reference_{content}_1', {})) + 1
                    ref_index = int(count_module(
                        f'reference_{content}_6', {})) + 1
                    result = insert_one_module(f'reference_{content}_6', {
                        'email': "",
                        'ver_code': "",
                        'verified': "No",
            ...
            if record_count == 0:
                for item in data:
                    ...
                    if item != "action":
                        if f'_{x}' in item:
                            ...
                            insert_dict[item.replace(f'_{x}', '')] = data_converter(
                                item, data[item])
                ...
                insert_dict['_id'] = ObjectId()
                if x == 2:
                    insert_dict['section'] = data['department_2'] + \
                        '-' + data['section_2']
                if x == 3:
                    insert_dict['location'] = data['area_3'] + \
                        '-' + data['location_3']
                if x == 4:
...
def update_login_details(data):
    update_dict = db['reference_employee_4'].find_one(
        {"index": session['index'], "meta.is_deleted": False})
    ...
    for item in update_dict:
        try:
            ...
            pass
    update_dict['password'] = hashlib.sha1(
...
def insert_reference_leave_credit(data, index):
    ...
    if int(data['ref_index']) == 0:
        ...
        find_rec = find_one_module('reference_leave_credit', {
                                   'ref_index': int(data['ref_index'])}, {'_id': 0, 'applied': 1})
        if find_rec['applied'][:10] == data['applied']:
            update_dict.update({'meta.reason_edit': data['reason_edit']})
            result = update_one_module(f'reference_leave_credit', {
                                       'ref_index': int(data['ref_index'])}, update_dict)
        else:
            result = insert_one_module(f'reference_leave_credit', insert_dict)
...
def delete_reference(data, content, index):
    if '3' in content or '2' in content or 'leave_credit' in content:
        update_dict = db[f'reference_{content}'].find_one(
            {"ref_index": int(index), "meta.is_deleted": False}, {"meta": 1, "_id": 0})
    else:
        update_dict = db[f'reference_{content}'].find_one(
            {"index": int(index), "meta.is_deleted": False}, {"meta": 1, "_id": 0})
    ...
    if '3' in content or '2' in content or 'leave_credit' in content:
        result = update_one_module(f'reference_{content}', {"ref_index": int(
            index), "meta.is_deleted": False}, update_dict)
    else:
        result = update_one_module(f'reference_{content}', {"index": int(
            index), "meta.is_deleted": False}, update_dict)
...
def upload_leave_credit(data):
    ...
    for data in reader:
        ...
        find_rec = find_one_module(f'reference_employee_1',  {
                                   "SN": data['SN_1'], "meta.is_deleted": False}, {'index': 1, '_id': 0})
        if find_rec:
            index = find_rec['index']
            leave_credit_rec = find_one_module('reference_leave_credit', {
                                               'index': index, 'meta.is_deleted': False, 'applied': f"{convert_nan_to_date(data['applied'])} 00:00"}, {'meta': 0, '_id': 0, 'index': 0})
            if not leave_credit_rec:
                for item in data:
                    if '_1' not in item and not item == 'applied':
                        insert_dict[item] = convert_nan_to_zero(
                            data_converter(item, data[item]))
                    elif item == 'applied':
                        applied_date = convert_nan_to_date(data['applied'])
                        insert_dict[item] = f"{applied_date} 00:00"

                insert_dict['ref_index'] = int(
                    count_module(f'reference_leave_credit', {})) + 1
                insert_dict['index'] = index
                insert_dict['_id'] = ObjectId()

                result = insert_one_module(
                    f'reference_leave_credit', insert_dict)
